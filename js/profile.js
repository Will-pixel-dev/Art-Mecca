...
class Profile {
  ...
  renderProfile() {
    ...
    if (artistTitleEl) {
      ...
      artistTitleEl.textContent =
        typeLabels[specialty] || specialty || "Digital Artist";
    }

    // Stats
    const stats = data.stats || {};
    document.querySelector('[data-stat="artworks"]').textContent =
      stats.artworks || 0;
    document.querySelector('[data-stat="followers"]').textContent =
      stats.followers || 0;
    document.querySelector('[data-stat="following"]').textContent =
      stats.following || 0;
    document.querySelector('[data-stat="likes"]').textContent =
      stats.totalLikes || 0;
  ...
  renderSocialLinks(links) {
    ...
    if (!hasLinks && !this.isOwnProfile) {
      container.innerHTML =
        '<span style="color:rgba(255,255,255,0.15);font-size:0.7rem;font-family:Share Tech Mono,monospace;">No social links</span>';
    }
  }
  ...
  renderBadge(badge) {
    ...
    if (badge.specialties && badge.specialties.length > 0) {
      specialtiesEl.innerHTML = badge.specialties
        .map((s) => `<span class="tag primary">${s.replace(/-/g, " ")}</span>`)
        .join("");
    } else {
      specialtiesEl.innerHTML = '<span class="tag">Not set</span>';
    }
    ...
    if (badge.mediums && badge.mediums.length > 0) {
      mediumsEl.innerHTML = badge.mediums
        .map((m) => `<span class="tag">${m.replace(/-/g, " ")}</span>`)
        .join("");
    } else {
      mediumsEl.innerHTML = '<span class="tag">Not set</span>';
    }
  ...
  updateThemeDependentElements() {
    ...
    if (searchInput) {
      ...
      searchInput.style.setProperty(
        "--placeholder-color",
        isLight ? "rgba(26,16,42,0.3)" : "rgba(255,255,255,0.3)",
      );
    }
  }
  ...
  loadAboutData() {
    ...
    fields.forEach((f) => {
      const el = document.getElementById(
        `about${f.charAt(0).toUpperCase() + f.slice(1)}`,
      );
      if (el) el.value = about[f] || "";
    });
    if (about.birthday) {
      const age = this.calculateAge(about.birthday);
      document.getElementById("aboutAge").textContent =
        age !== null ? `${age} years` : "—";
    }
    if (!this.isOwnProfile) {
      document
  ...
  async loadLiked(reset = false) {
    ...
    try {
      ...
      if (snapshot.empty) {
        ...
        const likeData = snapshot.docs.map((doc) => ({
          likeId: doc.id,
          ...doc.data(),
        }));
  ...
  async loadSaved(reset = false) {
    ...
    try {
      ...
      if (snapshot.empty) {
        ...
        const saveData = snapshot.docs.map((doc) => ({
          saveId: doc.id,
          ...doc.data(),
        }));
        const artworks = await Promise.all(
          saveData.map(async (save) => {
  ...
  async loadBadges() {
    ...
    try {
      ...
      if (this.userData?.badges) {
        this.userData.badges.forEach((b) =>
          badges.push({ icon: b.icon || "🏅", name: b.name || "Badge" }),
        );
      }
      const wins = await db
  ...
  async loadBlog() {
    ...
    try {
      ...
      if (!colSnapshot.empty) {
        collectionsGrid.innerHTML = colSnapshot.docs
          .map((doc) => {
            const data = doc.data();
      ...
      grid.innerHTML = postsSnapshot.docs
        .map((doc) => {
          const data = doc.data();
          return `
  ...
  loadCommission() {
    ...
    fields.forEach((f) => {
      const el = document.getElementById(
        `commission${f.charAt(0).toUpperCase() + f.slice(1)}`,
      );
  ...
  setupUploadModal() {
    ...
    nsfwToggle.addEventListener("change", () => {
      document.getElementById("nsfwCategory").style.display = nsfwToggle.checked
        ? "block"
        : "none";
    });
    ...
    tagsInput.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        ...
        if (
          tag &&
          this.uploadModalTags.length < 10 &&
          !this.uploadModalTags.includes(tag)
        ) {
  ...
  setupEventListeners() {
    ...
    document
      ...
      .addEventListener("click", () => this.saveAbout());
    document
      .getElementById("saveCommissionBtn")
      .addEventListener("click", () => this.saveCommission());

    // Social edit
    document
      .getElementById("editSocialBtn")
      .addEventListener("click", () => this.openSocialModal());

    // Badge edit
    document
      .getElementById("editBadgeBtn")
      .addEventListener("click", () => this.openBadgeModal());

    // Shadow
    document
      .getElementById("shadowBtn")
      .addEventListener("click", () => this.toggleShadow());
    ...
    document
      ...
      .addEventListener("click", () => this.submitUpload());
    this.setupUploadModal();

    // Collection modal
    document
      .getElementById("newCollectionBtn")
      .addEventListener("click", () => this.openCollectionModal());
    document
      .getElementById("closeCollectionModal")
      .addEventListener("click", () => this.closeCollectionModal());
    document
    ...
    // Portfolio
    document
      .getElementById("sharePortfolioBtn")
      .addEventListener("click", () => this.sharePortfolio());
    document
      .getElementById("copyPortfolioBtn")
      .addEventListener("click", () => this.copyPortfolioLink());
    document
      .getElementById("switchProfileBtn")
      .addEventListener("click", () => this.switchToProfile());
    document
      .getElementById("togglePortfolioBtn")
    ...
    document
      .getElementById("cvRemoveBtn")
      .addEventListener("click", () => this.removeCV());
    document
      .getElementById("cvDownloadBtn")
      .addEventListener("click", () => this.downloadCV());

    // Social modal
    document
      .getElementById("closeSocialModal")
      .addEventListener("click", () => this.closeSocialModal());
    document
      .getElementById("cancelSocialBtn")
      .addEventListener("click", () => this.closeSocialModal());
    document
      .getElementById("saveSocialBtn")
      .addEventListener("click", () => this.saveSocialLinks());

    // Badge modal
    document
      .getElementById("closeBadgeModal")
      .addEventListener("click", () => this.closeBadgeModal());
    document
      .getElementById("cancelBadgeBtn")
      .addEventListener("click", () => this.closeBadgeModal());
    document
      .getElementById("saveBadgeBtn")
      .addEventListener("click", () => this.saveBadge());
  ...
  async uploadLayer(file, type) {
    ...
    const removeBtn =
      type === "background"
        ...
        : document.getElementById("overlayRemove");
  ...
  populateSpecialties(type) {
    ...
    if (specs.length === 0) {
      specGrid.innerHTML =
        '<div class="empty-message">Select an artist type above to see specialties</div>';
    } else {
    ...
    if (meds.length === 0) {
      medGrid.innerHTML =
        '<div class="empty-message">Select an artist type above to see mediums</div>';
    } else {
  ...
  async saveBadge() {
    ...
    const specialties = Array.from(
      document.querySelectorAll("#specialtiesGrid input:checked"),
    ).map((cb) => cb.value);
    const mediums = Array.from(
      document.querySelectorAll("#mediumGrid input:checked"),
    ).map((cb) => cb.value);
  ...
  async openCollectionModal() {
    ...
    try {
      ...
      if (snapshot.empty) {
        select.innerHTML =
          '<p style="color:rgba(255,255,255,0.2);font-size:0.8rem;">No artworks found</p>';
      } else {
        select.innerHTML = snapshot.docs
          .map((doc) => {
            const data = doc.data();
            return `
      ...
      select.innerHTML =
        '<p style="color:rgba(255,255,255,0.2);font-size:0.8rem;">Error loading artworks</p>';
    }
  ...
  async saveCollection() {
    ...
    const selected = Array.from(
      document.querySelectorAll("#collectionArtworks input:checked"),
    ).map((cb) => cb.value);
  ...
  openBlockModal() {
    ...
    if (
      confirm(
        `Block ${this.userData?.fullname || "this user"}? They won't be able to interact with you.`,
      )
    ) {
      this.confirmBlock();
    }
