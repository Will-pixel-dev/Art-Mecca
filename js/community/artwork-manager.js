...
class ArtworkManager {
  ...
  async uploadArtwork(artworkData) {
    try {
      ...
      if (isNSFW) {
        ...
        if (!isAdult) {
          throw new Error(
            "You must be 18+ to upload mature content. Please verify your age in your profile.",
          );
        }
        if (!nsfwCategory) {
  ...
  async notifyModerators(artworkId, artwork) {
    try {
      ...
      if (notifications.length > 0) {
        ...
        await batch.commit();
        console.log(
          `Notified ${notifications.length} moderators about NSFW upload`,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (artwork.isNSFW && currentUserId) {
        ...
        if (userDoc.exists) {
          ...
          if (!isAdult) {
            throw new Error(
              "This artwork contains mature content and requires age verification",
            );
          }
        } else {
  ...
  async removeFromModerationQueue(artworkId) {
    try {
      ...
      const snapshot = await queueRef.where("artworkId", "==", artworkId).get();
