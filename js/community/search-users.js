...
class SearchUsers {
  ...
  updateThemeElements() {
    ...
    document.querySelectorAll('.filter-chip.active, .badge-filter-chip.active').forEach(el => {
  ...
  applyFilters() {
    ...
    if (this.searchQuery) {
      ...
      filtered = filtered.filter(user => {
        ...
        const basicMatch = user.fullname.toLowerCase().includes(query) ||
  ...
  renderUsers() {
    ...
    document.querySelectorAll('.user-card').forEach(card => {
      card.addEventListener('click', (e) => {
        if (!e.target.closest('.shadow-btn-small') &&
  ...
  createUserCard(user) {
    ...
    const bio = user.bio && user.bio.length > 100 ? user.bio.substring(0, 100) + '...' : user.bio || 'No bio yet.';
