...
class ChallengeVoting {
    ...
    async vote(submissionId, voteType) {
        ...
        try {
            const submissionRef = this.db.collection('challengeSubmissions').doc(submissionId);
            ...
            if (!voteQuery.empty) {
                ...
                if (oldVote === voteType) {
                    ...
                    await submissionRef.update({
                        votes: firebase.firestore.FieldValue.increment(voteType === 'up' ? 2 : -2)
                    });
    ...
    async getChallengeSubmissions(challengeId) {
        try {
            const snapshot = await this.db.collection('challengeSubmissions')
    ...
    async getChallengeWinners(limit = 20) {
        try {
            const snapshot = await this.db.collection('challengeWinners')
