...
class CommunityGallery {
    ...
    try {
      ...
      if (this.loadMoreContainer) {
        this.loadMoreContainer.style.display =
          this.hasMore && display.length > 0 ? "block" : "none";
      }
      ...
      if (error.message && error.message.includes("index")) {
        this.showToast(
          "⚠️ Firestore index required. Check console for link.",
          "error",
        );
  ...
  createCard(art) {
    try {
      ...
      const editButton =
        this.currentUser?.uid === art.artistId
  ...
  async handleLike(artworkId, btn) {
    ...
    try {
      ...
      const likeRef = this.db
        .collection("likes")
        .doc(`${artworkId}_${this.currentUser.uid}`);
    ...
    this.searchInput.addEventListener("input", (e) => {
      ...
      timeout = setTimeout(() => {
        ...
        if (this.searchClearBtn) {
          this.searchClearBtn.style.display = this.searchQuery
            ? "block"
            : "none";
        }
    ...
    if (user) {
      ...
      try {
        ...
        if (userDoc.exists) {
          ...
          const profilePic =
            data.profilePicture || data.photoURL || data.avatarUrl;
  ...
  setupThemeControls() {
    ...
    if (hudBtn) {
      hudBtn.addEventListener("click", () => {
        ...
        this.renderGallery(filtered);
        this.showToast(
          document.body.classList.contains("hud-mode")
            ? "🔮 HUD Mode ON"
            : "HUD Mode OFF",
        );
      });
