...
class ProfileHero {
  ...
  setupLayerControls() {
    ...
    const existingControls = document.querySelector('.layer-controls-container');
    if (existingControls) {
    ...
    toggleBtn.addEventListener('click', () => {
        ...
        controlsWrapper.style.display = this.layerControlsVisible ? 'flex' : 'none';
        toggleBtn.style.background = this.layerControlsVisible ? 'rgba(254, 103, 234, 0.3)' : 'rgba(0, 0, 0, 0.6)';
        toggleBtn.style.borderColor = this.layerControlsVisible ? '#fe67ea' : 'rgba(255, 255, 255, 0.1)';
    });
    ...
    updateLayerControlsVisibility() {
        ...
        controls.forEach(wrapper => {
            ...
            if (removeBtn) {
                if ((layer === 'layer1' && this.userData?.heroBackground) ||
    ...
    renderBadge() {
        ...
        if (!badge) {
            ...
            if (badgeSpecialties) badgeSpecialties.innerHTML = '<span class="badge-tag">Not set</span>';
            if (badgeMediumTags) badgeMediumTags.innerHTML = '<span class="badge-tag">Not set</span>';
        ...
        badgeArtistType.textContent = typeLabels[badge.artistType] || badge.artistType || 'Not set';
        ...
        if (badgeIcon) badgeIcon.className = typeIcons[badge.artistType] || 'fas fa-palette';

        if (badgeSpecialties) {
            if (badge.specialties && badge.specialties.length > 0) {
                badgeSpecialties.innerHTML = badge.specialties
                    .map(s => `<span class="badge-tag primary-tag">${this.formatSpecialtyLabel(s)}</span>`)
                    .join('');
            } else {
        ...
        if (badgeMediumTags) {
            if (badge.mediums && badge.mediums.length > 0) {
                badgeMediumTags.innerHTML = badge.mediums
                    .map(m => `<span class="badge-tag">${this.formatMediumLabel(m)}</span>`)
                    .join('');
            } else {
...
updateProfileAvatar() {
    ...
    const displayName = window.avatarManager?.getDisplayName() || this.userData?.fullname || 'Artist';
    ...
    filterSpecialties(artistType) {
        ...
        groups.forEach(group => {
            if (artistType === '') {
                ...
                group.style.display = group.dataset.type === artistType ? 'block' : 'none';
            }
        });
        ...
        mediumGroups.forEach(group => {
            if (artistType === '') {
                group.style.display = 'block';
            } else if (artistType === 'digital' || artistType === '3d' || artistType === 'animation') {
                group.style.display = group.dataset.type === 'digital' || group.dataset.type === 'platform' ? 'block' : 'none';
            } else if (artistType === 'traditional' || artistType === 'photography') {
                group.style.display = group.dataset.type === 'traditional' || group.dataset.type === 'platform' ? 'block' : 'none';
            } else {
                group.style.display = 'block';
            }
        });
    ...
    populateBadgeForm() {
        ...
        document.querySelectorAll('#specialtiesGrid input[type="checkbox"]').forEach(cb => {
            cb.checked = specialties.includes(cb.value);
        });

        const mediums = badge.mediums || [];
        document.querySelectorAll('#mediumGrid input[type="checkbox"]').forEach(cb => {
    ...
    async saveBadge() {
        ...
        const specialtyCheckboxes = document.querySelectorAll('#specialtiesGrid input[type="checkbox"]:checked');
        ...
        const mediumCheckboxes = document.querySelectorAll('#mediumGrid input[type="checkbox"]:checked');
        ...
        try {
            await db.collection('users').doc(this.userId).update({ badge: badgeData });
    ...
    renderProfile() {
       ...
       this.setupAvatarUpload();
        ...
        if (bioText) bioText.textContent = data.bio || 'No bio yet. Click to expand.';
    ...
    renderSocialLinks(data) {
        ...
        const socialPlatforms = [
            { id: 'instagram', icon: 'fa-instagram', url: data.socialLinks?.instagram },
            { id: 'tiktok', icon: 'fa-tiktok', url: data.socialLinks?.tiktok },
            { id: 'youtube', icon: 'fa-youtube', url: data.socialLinks?.youtube },
            { id: 'pinterest', icon: 'fa-pinterest', url: data.socialLinks?.pinterest },
        ...
        if (!hasLinks && !this.isOwnProfile) {
            ...
            placeholder.style.cssText = 'color: rgba(255,255,255,0.3); font-size: 0.8rem;';
    ...
    setLayer1Content(url) {
        ...
        const isFirebaseVideo = url.includes('video') || url.includes('.mp4') || url.includes('.webm');
...
setLayer2Content(url) {
    ...
    async uploadLayerFile(file, layer) {
        ...
        if (loadingMsg) {
            ...
            loadingMsg.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Uploading...';
        }
        ...
        try {
            ...
            this.showToast(`✅ ${layer === 'layer1' ? 'Background' : 'Overlay'} updated successfully!`);
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('.hero-right-sidebar .sidebar-icon').forEach(icon => {
...
setupNavScrollCollapse() {
    ...
    setupBioToggle() {
        const bio = document.querySelector('.hero-bio-left') || document.querySelector('.hero-bio');
        if (!bio) return;
        ...
        try {
            ...
            if (!existing.empty) {
                ...
                await shadowsRef.add({
                    ...
                    shadowerName: this.currentUser.displayName || this.currentUser.email.split('@')[0],
                ...
                btn.style.background = 'linear-gradient(135deg, rgba(254,103,234,0.3), rgba(99,219,238,0.3))';
