...
class CommentSystem {
    ...
    try {
      ...
      snapshot.forEach((doc) => {
        ...
        this.allUsers.push({
          uid: doc.id,
          username:
            data.username || data.displayName || data.fullname || "User",
          displayName:
            data.displayName || data.fullname || data.username || "User",
        });
      });
  ...
  checkRateLimit() {
    ...
    if (this.commentCountThisMinute >= this.maxCommentsPerMinute) {
      this.showToast(
        `Please wait a moment before posting more comments (${this.maxCommentsPerMinute} per minute)`,
        "warning",
      );
      return false;
    }
    ...
    if (now - this.lastCommentTime < this.commentCooldown) {
      const waitTime = Math.ceil(
        (this.commentCooldown - (now - this.lastCommentTime)) / 1000,
      );
      this.showToast(
        `Please wait ${waitTime} seconds before commenting again`,
        "warning",
      );
      return false;
    }
  ...
  setupMentionAutocomplete() {
    ...
    this.commentInput.addEventListener("keydown", (e) => {
      ...
      const items = this.suggestionDropdown.querySelectorAll(
      ...
      if (e.key === "ArrowDown") {
        ...
        if (
          ...
          items[this.activeSuggestionIndex]
        ) {
          e.preventDefault();
  ...
  showMentionSuggestions(searchTerm) {
    ...
    this.suggestionDropdown.innerHTML = suggestions
      .map(
        (user) => `
    ...
    // Add click handlers
    this.suggestionDropdown
      .querySelectorAll(".mention-suggestion-item")
      .forEach((item) => {
        ...
        item.addEventListener("mouseenter", () => {
          const allItems = this.suggestionDropdown.querySelectorAll(
            ".mention-suggestion-item",
          );
  ...
  setupEmojiPicker() {
    ...
    this.emojiBtn.addEventListener("click", (e) => {
      ...
      console.log(
        ...
        this.emojiPicker.classList.contains("visible"),
      );
    });
  ...
  async loadComments() {
    ...
    try {
      ...
      this.comments =
        this.page === 0 ? newComments : [...this.comments, ...newComments];
  ...
  async submitComment() {
    ...
    try {
      if (this.userCache[this.currentUser.uid]) {
        ...
        userAvatar =
          cached.profilePicture || cached.photoURL || cached.avatarUrl || null;
        userDisplayName =
          cached.displayName ||
          cached.fullname ||
          cached.username ||
    ...
    try {
      ...
      if (text !== filteredText) {
        this.showToast(
          "Your comment was filtered for inappropriate language",
          "warning",
        );
      }
  ...
  async handleMentions(text) {
    ...
    for (const mention of mentions) {
      ...
      const mentionedUser = this.allUsers.find(
      ...
      if (mentionedUser && mentionedUser.uid !== this.currentUser.uid) {
        try {
          console.log(
            `🔔 Creating notification for @${username} (${mentionedUser.uid})`,
          );
  ...
  replyToComment(commentId, userName) {
    ...
    this.commentInput.dispatchEvent(new Event("input"));
    this.commentInput.setSelectionRange(
      this.commentInput.value.length,
      this.commentInput.value.length,
    );
  ...
  async deleteComment(commentId) {
    ...
    if (
      !this.currentUser ||
      (comment.userId !== this.currentUser.uid && !this.isOwner)
    ) {
  ...
  renderComments() {
    ...
    const activeComments = this.comments.filter(
      (c) => c.status === "active" || c.status === "deleted",
    );
  ...
  renderCommentItem(comment, isReply = false) {
    ...
    const isArtworkOwner =
      this.artworkArtistId && comment.userId === this.artworkArtistId;
    ...
    const editedBadge = comment.editedAt
      ...
      : "";
    const ownerBadge = isArtworkOwner
  ...
  attachCommentEvents() {
    ...
    document
      .querySelectorAll(".comment-avatar-small, .comment-username")
      .forEach((el) => {
        el.addEventListener("click", (e) => {
          e.stopPropagation();
  ...
  formatTimeAgo(date) {
    ...
    const now = Date.now();
    const diff =
      now - (date instanceof Date ? date.getTime() : new Date(date).getTime());
