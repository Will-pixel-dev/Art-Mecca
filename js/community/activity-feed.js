...
class ActivityFeed {
    ...
    updateThemeElements() {
        ...
        document.querySelectorAll('.tab-btn.active, .period-btn.active').forEach(el => {
    ...
    async loadShadowingList() {
        try {
            ...
            document.getElementById('shadowCount').textContent = this.shadowingList.length;
    ...
    renderFeed() {
        ...
        document.getElementById('recentCount').textContent = Math.min(filteredActivities.length, 5);
        ...
        document.querySelectorAll('.feed-item').forEach(item => {
            item.addEventListener('click', (e) => {
                if (e.target.closest('.feed-image-preview') || e.target.closest('.feed-tag')) return;
            ...
            <div class="feed-item" data-id="${activity.id}">
                ...
                <div class="feed-content">
                    ...
                    ${activity.tags && activity.tags.length > 0 ? `
                        <div class="feed-tags">
                            ${activity.tags.slice(0, 5).map(tag => `<span class="feed-tag">#${this.escapeHtml(tag)}</span>`).join('')}
                        </div>
                    ` : ''}
