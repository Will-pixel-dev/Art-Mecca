...
class ChallengesSystem {
        ...
        this.yearlyThemes = [
            {
                ...
                description: "Document your artistic journey and personal growth over a year. Create 12 pieces that show your evolution as an artist.",
                tags: ["growth", "journey", "transformation", "evolution"]
            },
            {
                title: "Chronicles of Time: Seasonal Diary",
                description: "Create a visual diary of the seasons and passing of time. Capture the beauty of each season through your art.",
                tags: ["time", "seasons", "reflection", "nature"]
            },
            {
                title: "52 Weeks of Creative Exploration",
                description: "52 weeks of creative exploration, one piece per week. Push your boundaries and try new techniques.",
        ...
        try {
            ...
            for (const challenge of this.challenges) {
                ...
                if (!existingIds.has(challenge.id)) {
                    await this.db.collection('challenges').doc(challenge.id).set({
                        ...challengeData,
                        createdAt: firebase.firestore.FieldValue.serverTimestamp()
                    });
                } else {
                    await this.db.collection('challenges').doc(challenge.id).update(challengeData);
                }
            }
    ...
    async checkAndCompleteChallenges() {
        try {
            ...
            if (completedCount > 0) {
                console.log(`🏆 Completed ${completedCount} challenges with winners selected!`);
                await this.loadChallenges();
                this.renderChallenges();
            }
    ...
    async selectChallengeWinner(challengeId) {
        try {
            const submissionsSnapshot = await this.db.collection('challengeSubmissions')
            ...
            const artworkDoc = await this.db.collection('artworks').doc(winner.artworkId).get();
            if (artworkDoc.exists) {
                await this.db.collection('artworks').doc(winner.artworkId).update({
                    isWinner: true,
            ...
            await this.awardPointsToUser(winner.userId, winBonus, `🏆 Won challenge: ${challengeData.title || 'Challenge'}`);

            if (submissions.length > 1) {
                const runnerUp = submissions[1];
                await this.db.collection('challengeSubmissions').doc(runnerUp.id).update({
                    status: 'runner-up',
                    rank: 2
                });
                await this.awardPointsToUser(runnerUp.userId, Math.floor(winBonus * 0.5), `🥈 Runner-up in: ${challengeData.title || 'Challenge'}`);
            }
            ...
            console.log(`🏆 Winner selected for challenge ${challengeId}: ${winner.userName || winner.userId}`);
    ...
    getIntuitChallenges() {
        if (window.communityHubInstance) {
            try {
                const intuitChallenges = window.communityHubInstance.getIntuitChallenges();
    ...
    generateFallbackIntuitChallenges() {
        ...
        const weeklyTheme = this.weeklyThemes[(weekNumber - 1) % this.weeklyThemes.length];
        ...
        const intuitWeekly = {
            ...
            endDate: weeklyEnd,
            prize: 'Champion Badge + Social Media Feature + Pinned in Community + 250 Points',
            prizeValue: 250,
            participants: 0,
            submissions: 0,
            status: 'active',
        ...
        const monthlyTheme = this.monthlyThemes[monthIndex % this.monthlyThemes.length];
        ...
        const intuitMonthly = {
            ...
            endDate: monthlyEnd,
            prize: 'Master Badge + Homepage Feature + Pick Next Monthly Theme + 500 Points',
            prizeValue: 500,
            participants: 0,
            submissions: 0,
            status: 'active',
        ...
        const intuitYearly = {
            ...
            endDate: yearlyEnd,
            prize: 'Grand Master Badge + Featured Artist + Solo Exhibition + Design a Challenge + 1000 Points',
            prizeValue: 1000,
            participants: 0,
            submissions: 0,
            status: 'active',
    ...
    async leaveChallenge(challengeId) {
        ...
        if (!confirm(`Are you sure you want to leave "${challenge.title}"? You'll lose your progress and any points earned from this challenge.`)) {
            return;
        }

        try {
            if (userChallenge.docId) {
                await this.db.collection('userChallenges').doc(userChallenge.docId).delete();
            } else {
                const snapshot = await this.db
                    .collection('userChallenges')
                    .where('userId', '==', this.currentUser.uid)
    ...
    async submitArtwork(challengeId) {
        ...
        if (!this.userChallenges.has(challengeId)) {
            this.showToast('Please join the challenge first before submitting.', 'warning');
            return;
        }
    ...
    createChallengeCard(challenge) {
        ...
        const endDateForCountdown = endDate instanceof Date ? endDate : new Date(endDate);
        ...
        const accentColor = challenge.isIntuit ? '#ff00ea' : colorMap[challenge.type] || '#ffffff';
        ...
        const endDateString = endDateForCountdown instanceof Date ? endDateForCountdown.toISOString() : '';
    ...
    updateThemeElements() {
        ...
        document.querySelectorAll('.challenge-countdown .countdown-text').forEach(el => {
            el.style.color = color1;
        });
        ...
        document.querySelectorAll('.challenge-prize .prize-badge-icon').forEach(el => {
            el.style.background = gradient;
        });
...
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        ...
        console.log(`✅ ChallengesSystem initialized with ${challengesSystemInstance.challenges.length} challenges`);
    }, 500);
});
