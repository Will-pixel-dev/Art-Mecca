...
class AccountSettings {
  ...
  populateForm() {
    ...
    document.getElementById("showEmail").checked =
      this.userData?.showEmail || false;
    document.getElementById("privateProfile").checked =
      this.userData?.privateProfile || false;
    document.getElementById("showNSFW").checked =
      this.userData?.nsfwPreferences?.enabled || false;

    // Notifications
    document.getElementById("emailNotifications").checked =
      this.userData?.notifications?.email !== false;
    document.getElementById("pushNotifications").checked =
      this.userData?.notifications?.push || false;
    document.getElementById("likeNotifications").checked =
      this.userData?.notifications?.likes !== false;
    document.getElementById("commentNotifications").checked =
      this.userData?.notifications?.comments !== false;
    document.getElementById("shadowNotifications").checked =
      this.userData?.notifications?.shadows !== false;
  ...
  async saveNSFWCreatorStatus(enabled) {
    try {
      ...
      if (enabled) {
        this.showToast(
          "🔞 NSFW Creator mode enabled! Your profile is now age-gated.",
          "success",
        );
      } else {
        this.showToast(
          "NSFW Creator mode disabled. Your profile is now public.",
          "success",
        );
      }
      ...
      this.showToast(
        "Error updating NSFW Creator status: " + error.message,
        "error",
      );
  ...
  async deleteAccount() {
    if (
      !confirm(
        "⚠️ Are you sure you want to delete your account? This action is PERMANENT and cannot be undone!",
      )
    ) {
      return;
    }
    ...
    if (
      !confirm(
        "All your artwork, comments, and data will be permanently deleted. Are you absolutely sure?",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (error.code === "auth/requires-recent-login") {
        this.showToast(
          "Please log out and log back in to confirm your identity",
          "error",
        );
      } else {
