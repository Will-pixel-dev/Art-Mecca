...
function showToast(icon, message) {
    ...
    if (newsPrevBtn) {
        newsPrevBtn.addEventListener('click', () => goToNewsSlide(currentNewsSlide - 1));
    }
    if (newsNextBtn) {
        newsNextBtn.addEventListener('click', () => goToNewsSlide(currentNewsSlide + 1));
    }
    ...
    // Auto-play
    let newsInterval = setInterval(() => goToNewsSlide(currentNewsSlide + 1), 8000);
    ...
    if (newsContainer) {
        ...
        newsContainer.addEventListener('mouseleave', () => {
            newsInterval = setInterval(() => goToNewsSlide(currentNewsSlide + 1), 8000);
        });
    }
    ...
    function initWinnersCarousel() {
        ...
        function goToSlide(index) {
            ...
            if (winnersCarousel) {
                ...
                winnersCarousel.style.transition = 'transform 0.6s cubic-bezier(0.34, 1.56, 0.64, 1)';
            }
            ...
            if (winnersNextBtn) {
                ...
                winnersNextBtn.style.cursor = index === maxIndex ? 'not-allowed' : 'pointer';
            }
        }
    ...
    async function loadStats() {
        try {
            ...
            if (elements.heroTotalArtworks) elements.heroTotalArtworks.textContent = artworks;
            if (elements.heroTotalArtists) elements.heroTotalArtists.textContent = users;
            if (elements.heroActiveChallenges) elements.heroActiveChallenges.textContent = challenges;
            ...
            if (elements.activeChallenges) elements.activeChallenges.textContent = challenges;
            ...
            const fallback = { artworks: '10K+', users: '5K+', challenges: '12', likes: '25K+' };
    ...
    async function loadRecentArtworks() {
        ...
        try {
            ...
            usersSnapshot.forEach(doc => {
                const data = doc.data();
                userMap[doc.id] = {
                    displayName: data.username ? `@${data.username}` : (data.fullname || 'Artist')
                };
            });
            ...
            snapshot.forEach(doc => {
                ...
                const userInfo = userMap[userId] || { displayName: data.artistName || 'Unknown Artist' };
                const displayName = userInfo.displayName || data.artistName || 'Unknown Artist';
    ...
    function generateFallbackChallenges() {
        ...
        const dailyPrompts = [
            { title: "At the Top of the World", desc: "Create artwork depicting the highest peaks, both literal and metaphorical." },
            { title: "Breathe", desc: "Capture the essence of breath, life, and moments of calm." },
            { title: "Fallen", desc: "Explore themes of descent, loss, or transformation." }
        ];

        const weeklyThemes = [
            { title: "Urban Jungle", desc: "Blend city architecture with natural elements." },
            { title: "Golden Hour Glow", desc: "Capture the magic of the golden hour." },
            { title: "Silent Stories", desc: "Create artwork that tells a story without words." }
        ];

        const monthlyThemes = [
            { title: "Mythical Realms Reimagined", desc: "Take a classic mythical creature and give it a fresh twist." },
            { title: "Urban Echoes", desc: "Capture the hidden stories of the city." },
            { title: "Ethereal Visions", desc: "Explore the space between dreams and reality." }
        ];
    ...
    function renderChallenges(container, challenges) {
        ...
        challenges.forEach(challenge => {
            ...
            const typeDisplay = challenge.type ? challenge.type.charAt(0).toUpperCase() + challenge.type.slice(1) : 'Active';
    ...
    async function loadActivityFeed() {
        ...
        try {
            ...
            let snapshot = await db.collection('activity')
                .orderBy('createdAt', 'desc')
                .limit(10)
                .get();

            if (snapshot.empty) {
                const artworksSnap = await db.collection('artworks')
                    .where('status', '==', 'published')
                    .orderBy('createdAt', 'desc')
                    .limit(10)
                    .get();
                ...
                artworksSnap.forEach(doc => {
                    const data = doc.data();
                    const userId = data.artistId || data.userId || data.uid;
                    const userInfo = userMap[userId] || { displayName: data.artistName || 'Someone' };
            ...
            snapshot.forEach(doc => {
                ...
                const userInfo = userMap[userId] || { displayName: data.userName || data.artistName || 'Someone' };
                const displayName = userInfo.displayName || data.userName || data.artistName || 'Someone';

                activities.push({
                    icon: data.icon || '📢',
                    message: data.message ? data.message.replace(/\{user\}/g, displayName) : `${displayName} ${data.action || 'was active'}`,
                    time: data.createdAt,
                    userId: userId,
...
async function loadFeaturedArtists() {
    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            let submissions = 0;
            try {
                const submissionsSnap = await db.collection('challengeSubmissions')
                    .where('userId', '==', userId)
                    .get();
            ...
            } else if (data.profilePicture && typeof data.profilePicture === 'string' && data.profilePicture.startsWith('http')) {
                avatarUrl = data.profilePicture;
            } else if (data.avatarUrl && typeof data.avatarUrl === 'string' && data.avatarUrl.startsWith('http')) {
                avatarUrl = data.avatarUrl;
            } else if (data.photoURL && typeof data.photoURL === 'string' && data.photoURL.startsWith('http')) {
                avatarUrl = data.photoURL;
            }

            // Format numbers with K for thousands
            const formattedArtworks = artworkCount >= 1000 ? (artworkCount / 1000).toFixed(1) + 'K' : artworkCount.toString();
            const formattedPoints = points >= 1000 ? (points / 1000).toFixed(1) + 'K' : points.toString();
            const formattedFollowers = followers >= 1000 ? (followers / 1000).toFixed(1) + 'K' : followers.toString();
    ...
    async function loadChallengeWinners() {
        ...
        try {
            ...
            try {
                const snapshot = await db.collection('challengeWinners')
                    .orderBy('createdAt', 'desc')
                    .limit(10)
                    .get();
    ...
    function generatePlaceholderWinners() {
        const names = ['CyberArtist', 'NeonDreamer', 'PixelWizard', 'GlitchMaster', 'VoidWalker', 'DigitalProphet', 'CrystalMage', 'StormChaser', 'LunarArtist', 'StarForger'];
        const usernames = ['@cyber_art', '@neon_dream', '@pixel_wiz', '@glitch_master', '@void_walker', '@digital_prophet', '@crystal_mage', '@storm_chaser', '@lunar_artist', '@star_forger'];
        const icons = ['🎨', '🖌️', '✨', '🌟', '🎭', '🌈', '💫', '🎯', '🌸', '⚡'];
        const colors1 = ['#ff00ea', '#58ebfe', '#ffc72e', '#ad03fc', '#ff00ea', '#4ff3a6', '#58ebfe', '#ff6b00', '#4ff3a6', '#ffc72e'];
        const colors2 = ['#ad03fc', '#4ff3a6', '#ff6b00', '#ff00ea', '#58ebfe', '#ffc72e', '#ad03fc', '#ff00ea', '#58ebfe', '#ff6b00'];
